...
$consulta = mysql_query("select imagen from tnoticias where id='$codigo'",$link);
...
mysql_query("DELETE FROM tnoticias WHERE id='$codigo' ",$link);
	unlink("../imagenes/noticias/".$imagen[0]);//para eliminar la imagen 

$my_error = mysql_error($link);
	}
	if($estado=='1')
	{
mysql_query("UPDATE tnoticias SET  estado='1' WHERE id='$codigo'",$link);
$my_error = mysql_error($link);
	}
	
	if($estado=='0')
	{
mysql_query("UPDATE tnoticias SET  estado='0' WHERE id='$codigo'",$link);
$my_error = mysql_error($link);
	}
...
$consulta ="SELECT id,nombre, fecha, estado FROM tnoticias ORDER BY id"; 
